...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname='Series publicly viewable' AND tablename='series'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname='Admins can manage series' AND tablename='series'
  ) THEN
