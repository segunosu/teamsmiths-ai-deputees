...
('Marketing Agile', 'marketing-agile', (SELECT id FROM categories WHERE slug = 'sales-acceleration'))
ON CONFLICT (slug) DO NOTHING;

-- Add new products for Software Development companies
INSERT INTO products (title, description, deliverables, timeline, base_price, tags, category_id, subcategory_id) VALUES 
(
  'Agile Sprint Setup',
...
(
  ...
  (SELECT id FROM subcategories WHERE slug = 'marketing-agile')
),
(
  ...
  (SELECT id FROM categories WHERE slug = 'sales-acceleration'),
  (SELECT id FROM subcategories WHERE slug = 'outbound')
),
(
